// SmoothScroll for websites v2.0
// Licensed under MIT
// Maintainer: Balazs Galambosi, Pulse: Michael Herf
...
function scrollArray(elem, left, top, delay){
    ...
    var step = function(){
        var now = +new Date, scrollX=0, scrollY=0;
        ...
        if(!left && !top) que=[];
        if(que.length) requestFrame(step);
        else pending=false;
    };
...
function keydown(event){
    ...
    if(/input|textarea|select|embed/i.test(target.nodeName) ||
       target.isContentEditable || event.defaultPrevented || modifier)
        return true;
    if(isNodeName(target,"button") && event.keyCode===key.spacebar) return true;

    var x=0, y=0, elem=overflowingAncestor(activeElement);
    var clientHeight = (elem==document.body) ? window.innerHeight : elem.clientHeight;
    var shift;
    ...
    switch(event.keyCode){
        ...
        case key.home: y=-elem.scrollTop; break;
        case key.end: y=elem.scrollHeight - elem.scrollTop - clientHeight; break;
...
function overflowingAncestor(el){
    var elems=[]; var rootScrollHeight = root.scrollHeight;
    do{
...
function addEvent(type, fn, bubble){
    if(typeof bubble==="object") bubble = bubble.passive !== undefined ? bubble : false;
...
var requestFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || function(callback){ window.setTimeout(callback,1000/60); };
...
/******************** ATTACH EVENTS ********************/
var isMouseWheelSupported = 'onwheel' in document || 'onmousewheel' in document;

if(isMouseWheelSupported){
    addEvent("mousedown", mousedown);
    addEvent("wheel", wheel, {passive:false});
